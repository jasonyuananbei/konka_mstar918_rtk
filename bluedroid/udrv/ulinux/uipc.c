...
static void uipc_flush_locked(tUIPC_CH_ID ch_id)
{
    ...
    switch(ch_id)
    {
        ...
        case UIPC_CH_ID_AV_AUDIO:
            ...
            break;
#ifdef BLUETOOTH_RTK_SCO
        case UIPC_CH_ID_SCO_CTRL:
            uipc_flush_ch_locked(UIPC_CH_ID_SCO_CTRL);
            break;

        case UIPC_CH_ID_SCO_OUT:
            uipc_flush_ch_locked(UIPC_CH_ID_SCO_OUT);
            break;

        case UIPC_CH_ID_SCO_IN:
            uipc_flush_ch_locked(UIPC_CH_ID_SCO_IN);
            break;
#endif
    }
}
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
#ifdef BLUETOOTH_RTK_VR
        case UIPC_CH_ID_RTKBT_VR_CTRL:
            uipc_setup_server_locked(ch_id, VOHOG_CTRL_PATH, p_cback);
            break;
...
#ifdef BLUETOOTH_RTK_SCO
        case UIPC_CH_ID_SCO_CTRL:
            uipc_setup_server_locked(ch_id, SCO_CTRL_PATH, p_cback);
            break;

        case UIPC_CH_ID_SCO_OUT:
            uipc_setup_server_locked(ch_id, SCO_OUT_PATH, p_cback);
            break;

        case UIPC_CH_ID_SCO_IN:
            uipc_setup_server_locked(ch_id, SCO_IN_PATH, p_cback);
            break;
#endif
    }
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
#ifdef BLUETOOTH_RTK_VR
UDRV_API BOOLEAN UIPC_RTKBT_VR_Send_noblock(tUIPC_CH_ID ch_id, UINT16 msg_evt, UINT8 *p_buf,
        UINT16 msglen)
{
    ...
    if (send(uipc_main.ch[ch_id].fd, p_buf, msglen, MSG_DONTWAIT) < 0)
    {
        BTIF_TRACE_ERROR1("failed to write (%s)", strerror(errno));
    } else
    {
