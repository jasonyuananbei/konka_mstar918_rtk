...
void l2c_link_check_send_pkts (tL2C_LCB *p_lcb, tL2C_CCB *p_ccb, BT_HDR *p_buf)
{
    ...
    L2CAP_TRACE_DEBUG2("cheng_@l2c_link_check_send_pkts l2cb.check_round_robin:%d single_write:%d",l2cb.check_round_robin,single_write);
    ...
    */
    if (l2cb.is_cong_cback_context)
        return;
    L2CAP_TRACE_DEBUG3 ("l2c_link_check_send_pkts p_lcb->link_xmit_quota = %d,p_lcb->sent_not_acked = %d,handle =%d",p_lcb->link_xmit_quota,p_lcb->sent_not_acked,p_lcb->handle);
    ...
    */
    if ( (p_lcb == NULL) || (p_lcb->link_xmit_quota == 0) )
    {
        ...
        L2CAP_TRACE_DEBUG1("l2c_link_check_send_pkts , l2cb.controller_xmit_window = %d",l2cb.controller_xmit_window);
        ...
        for (xx = 0; xx < MAX_L2CAP_LINKS; xx++, p_lcb++)
        {
            ...
            {
                L2CAP_TRACE_DEBUG0 ("cheng_@there is data to send in the link queue");
            ...
            {
                L2CAP_TRACE_DEBUG0 ("cheng_@there is data to send in the channel queue");
                l2c_link_send_to_lower (p_lcb, p_buf);
            }
        }
    ...
    {
        L2CAP_TRACE_DEBUG2("cheng_@partial_segment_being_sent = %d,  L2C_LINK_CHECK_POWER_MODE = %d" , p_lcb->partial_segment_being_sent, L2C_LINK_CHECK_POWER_MODE (p_lcb));
...
static BOOLEAN l2c_link_send_to_lower (tL2C_LCB *p_lcb, BT_HDR *p_buf)
{
...
#endif
    {
...
#endif
        {
            ...
            L2CAP_TRACE_DEBUG6 ("l2c_link_send_to_lower bdaddress: %x:%x:%x:%x:%x:%x",p_lcb->remote_bd_addr[0],p_lcb->remote_bd_addr[1],p_lcb->remote_bd_addr[2],p_lcb->remote_bd_addr[3],p_lcb->remote_bd_addr[4],p_lcb->remote_bd_addr[5]);
            L2CAP_TRACE_DEBUG1 ("local_cid : %d", p_buf->event);
    ...
    for (xx = 0; xx < num_handles; xx++)
    {
	    ...
	    p_lcb->get_num_of_complete = 1;
            ...
            if ( (p_lcb->acl_priority == L2CAP_PRIORITY_HIGH)
              ...
              && (l2cb.round_robin_unacked < l2cb.round_robin_quota) )
            {
              L2CAP_TRACE_DEBUG0 ("we are in the if before l2c_link_check_send_packets");
