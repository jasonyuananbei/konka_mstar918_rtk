...
void btm_sec_conn_req (UINT8 *bda, UINT8 *dc)
{
    ...
    bda[0], bda[1], bda[2],bda[3], bda[4], bda[5]);
    UINT32 cod = devclass2uint(dc);
    BTM_TRACE_EVENT1 ("cheng_dev_class = %d", cod);
    if( !btif_config_exist("Remote",bdstr,"LinkKey") && ((cod & 0x700) == 0x500) ) {
        BTM_TRACE_EVENT0 ("cheng_Security Manager: connect request from non-paired device");
        btsnd_hcic_reject_conn (bda, HCI_ERR_HOST_REJECT_DEVICE);
        return;
    }
#endif
