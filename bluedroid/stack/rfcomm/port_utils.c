...
{
    RFCOMM_TRACE_EVENT2 ("port_flow_control_peer enable:%d,count:%d",enable,count);
    if (!p_port->rfc.p_mcb)
        return;
    RFCOMM_TRACE_EVENT2 ("port_flow_control_peer enable : %d, count: %d",enable, count);
    ...
    if (p_port->rfc.p_mcb->flow == PORT_FC_CREDIT)
    {
        ...
        if (enable)
        {
            ...
            {
                RFCOMM_TRACE_EVENT1 ("port_flow_control_peer p_port->credit_rx:%d",p_port->credit_rx);
