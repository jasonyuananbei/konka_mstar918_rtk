...
{
    RFCOMM_TRACE_DEBUG2("rfc_port_sm_execute port event %d,rfc.state %d", event,p_port->rfc.state);
    if (!p_port)
    {
...
{
    RFCOMM_TRACE_WARNING1 ("rfc_port_sm_opened event is %d", event);
    switch (event)
    {
    case RFC_EVENT_OPEN:
        RFCOMM_TRACE_ERROR2 ("Port error state %d event %d", p_port->rfc.state, event);
    ...
    case RFC_EVENT_CLOSE:
        rfc_port_timer_start (p_port, RFC_DISC_TIMEOUT);
        RFCOMM_TRACE_WARNING1 ("rfc_port_sm_opened event is %d,before send_disc", event);
        rfc_send_disc (p_port->rfc.p_mcb, p_port->dlci);
        p_port->rfc.expected_rsp = 0;
        p_port->rfc.state = RFC_STATE_DISC_WAIT_UA;
        return;
