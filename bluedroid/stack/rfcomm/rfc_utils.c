...
#ifdef BLUETOOTH_RTK
static void port_resend_flow_control_peer(tPORT *p_port);
#endif
...
#ifdef BLUETOOTH_RTK
    case BTU_TTYPE_RFCOMM_CREDIT:
        RFCOMM_TRACE_EVENT0 ("rfc_credit restart send credit");
        port_resend_flow_control_peer((tPORT *)p_tle->param);
        break;
...
#ifdef BLUETOOTH_RTK
void rfc_credit_timer_start (tPORT *p_port, UINT16 timeout)
{
    RFCOMM_TRACE_EVENT1 ("rfc_credit_timer_start - timeout:%d", timeout);

    rfc_credit_ent.param = (UINT32)p_port;

    btu_start_timer (&rfc_credit_ent, BTU_TTYPE_RFCOMM_CREDIT, timeout);
}

void rfc_credit_timer_stop (tPORT *p_port)
{
    RFCOMM_TRACE_EVENT0 ("rfc_credit_timer_stop");

    btu_stop_timer (&rfc_credit_ent);
}

static void port_resend_flow_control_peer(tPORT *p_port)
{
    RFCOMM_TRACE_EVENT2 ("port_resend_flow_control_peer p_port->credit_rx:%d,p_port->credit_rx_max : %d",p_port->credit_rx,p_port->credit_rx_max);
    if (!p_port->rx.user_fc && (p_port->credit_rx_max >= p_port->credit_rx))
    {
        if(p_port->credit_rx == 6 || p_port->credit_rx == 10){
        ...
        rfc_credit_timer_start(p_port,BTU_TTYPE_RFCOMM_CREDIT_TIMEOUT);
    }
        }
