...
static struct a2dp_stream_out * command_out = NULL;
...
static void command_thread(void* param)
{
    ...
    do
    {
        ...
        {
            case 1:
                pthread_mutex_lock(&command_out->lock);
                start_audio_datapath(command_out);
                pthread_mutex_unlock(&command_out->lock);
                break;
            case 2:
                break;
            case 3:
        ...
        ERROR("A2DP_DELAY: command_id %d finished\n",command_id);
        command_id = 0;
        usleep(1000000);
    } while (1);
    return;
                         ...
                         size_t bytes)
{
    ...
    memset(cap_buf, 0, 0x100);

    if (out->state == AUDIO_A2DP_STATE_SUSPENDED)
    {
        gettimeofday(&tv1,NULL);
        if (pcm) pcm_read(pcm, cap_buf, bytes);
        gettimeofday(&tv2,NULL);
        timersub(&tv2,&tv1,&tv);
        ERROR("A2DP_DELAY7.out_write_delay  (%d:%d) %d:%d",bytes,tv.tv_sec,tv.tv_usec,tv1.tv_sec,tv1.tv_usec);
        ERROR("A2DP_DELAY7.out_write_delay %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n", \
    ...
    else if (out->state != AUDIO_A2DP_STATE_STARTED)
    {
        ...
        gettimeofday(&tv2,NULL);
        timersub(&tv2,&tv1,&tv);
        ERROR("A2DP_DELAY7.out_write_delay--->a2dp %d bytes!!! (%d:%d) %d:%d",bytes,tv.tv_sec,tv.tv_usec,tv1.tv_sec,tv1.tv_usec);
        ERROR("A2DP_DELAY7.out_write_delay %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n", \
            cap_buf[0x40],cap_buf[0x41],cap_buf[0x42],cap_buf[0x43],cap_buf[0x44],cap_buf[0x45],cap_buf[0x46],cap_buf[0x47], \
            cap_buf[0x48],cap_buf[0x49],cap_buf[0x4a],cap_buf[0x4b],cap_buf[0x4c],cap_buf[0x4d],cap_buf[0x4e],cap_buf[0x4f]);
        return -1;
...
// for fd not ready not to write fd
    if(AUDIO_SKT_DISCONNECTED == out->audio_fd)
    {
        gettimeofday(&tv1,NULL);
        if (pcm) pcm_read(pcm, cap_buf, bytes);
        gettimeofday(&tv2,NULL);
...
#endif
// MStar Android Patch End
    if (pthread_create(&command_thread_id, NULL, (void*)command_thread,out)!=0)
                     ...
                     hw_device_t** device)
{
    ...
    FNLOG();
    INFO(" A2dp_hw_module_version:rtk a2dp(mstar)");
