...
rtkbt_3dd_callbacks_t *rtkbt_3dd_hal_cbacks = NULL;
#endif
...
{
    ALOGI("init");
    ALOGI("before interface_ready");
    /* sanity check */
    if (interface_ready() == TRUE)
        return BT_STATUS_DONE;
    ALOGI("after interface_ready");
...
/*BOARD_HAVE_BLUETOOTH_RTK_3DD Begin*/
#ifdef BLUETOOTH_RTK_3DD
static int rtkbt_3dd_send_command(uint8_t* buf, uint8_t len)
{
    ALOGI(" < ===bluedroid 3dd === > send_3dd_command");
    /* sanity check */
    if (interface_ready() == FALSE)
        return BT_STATUS_NOT_READY;
    ...
    uint16_t opcode;
    opcode = (uint16_t)((buf[1] & 0x03ff)|(buf[0] << 10));

    return btif_rtkbt_3dd_send_command(opcode, buf+2 , len-2);
}

static int rtkbt_3dd_update_eir(void)
{
    ALOGI(" < ===bluedroid 3dd === > update_eir");

    /* sanity check */
    if (interface_ready() == FALSE)
        return BT_STATUS_NOT_READY;

    btif_rtkbt_3dd_update_eir();

    return BT_STATUS_SUCCESS;
}

static int rtkbt_3dd_init(rtkbt_3dd_callbacks_t* rtkbt_3dd_cbacks)
{
    ALOGI("< ===bluedroid 3dd === >init_3dd");
    if (interface_ready() == FALSE)
        return BT_STATUS_FAIL;

    rtkbt_3dd_hal_cbacks = rtkbt_3dd_cbacks;
...
/*BOARD_HAVE_BLUETOOTH_RTK_3DD Begin*/
#ifdef BLUETOOTH_RTK_3DD
    rtkbt_3dd_send_command,
    rtkbt_3dd_update_eir,
    rtkbt_3dd_init,
#endif
/*BOARD_HAVE_BLUETOOTH_RTK_3DD End*/
