...
static void bta_scan_results_cb (tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data)
{
    ...
    switch (event)
    {
        case BTA_DM_INQ_RES_EVT:
        {
            ...
            if ( cod == 0) {
                if (btif_storage_get_remote_device_property(&btif_cb.bd_addr, &properties[num_properties]) == BT_STATUS_SUCCESS)
                {
            ...
            BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                ...
                                &(p_data->inq_res.RTKBT_AUTOPAIR_txpower));
            num_properties++;

            BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_SUPPORTED, sizeof(int8_t),
                                &(p_data->inq_res.RTKBT_AUTOPAIR_classic_supported));
            num_properties++;

            if(p_data->inq_res.RTKBT_AUTOPAIR_classic_supported)
            {
                BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                    BT_PROPERTY_RTKBT_AUTOPAIR_CLASSIC_ADDR, sizeof(p_data->inq_res.RTKBT_AUTOPAIR_classic_addr),
                                    &(p_data->inq_res.RTKBT_AUTOPAIR_classic_addr));
                num_properties++;
            }

            BTIF_STORAGE_FILL_PROPERTY(&properties[num_properties],
                                BT_PROPERTY_RTKBT_AUTOPAIR_RCUID, sizeof(uint32_t),
                                &(p_data->inq_res.RTKBT_AUTOPAIR_rcuid));
            num_properties++;
