...
static int send_data_to_app(int fd, BT_HDR *p_buf)
{
    if(p_buf->len == 0)
        return SENT_ALL;
    APPL_TRACE_ERROR0("send_data_to_app before send");
    int sent = send(fd, (UINT8 *)(p_buf + 1) + p_buf->offset,  p_buf->len, MSG_DONTWAIT);
    APPL_TRACE_ERROR1("send_data_to_app after send %d", send);
    if(sent == p_buf->len)
        return SENT_ALL;
...
int bta_co_rfc_data_incoming(void *user_data, BT_HDR *p_buf)
{
    ...
    if(rs)
    {
        ...
        {
            int sent = send_data_to_app(rs->fd, p_buf);
            APPL_TRACE_DEBUG1("bta_co_rfc_data_incoming sent %d", sent);
            switch(sent)
            {
                case SENT_NONE:
                case SENT_PARTIAL:
    ...
    unlock_slot(&slot_lock);
    APPL_TRACE_DEBUG1("bta_co_rfc_data_incoming ret %d", ret);
