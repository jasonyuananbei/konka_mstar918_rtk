...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
            ...
            if(RTKBT_VR_get_dev_cb()){
                ...
                p_rpt = bta_hh_le_find_report_entry(RTKBT_VR_get_dev_cb(),
                                        p_data->api_write.srvc_id.id.inst_id,
                                        p_data->api_write.char_id.uuid.uu.uuid16,
                                        p_data->api_write.char_id.inst_id);
                if((p_rpt->rpt_id == rtk_voice_output_rpt_id) && (*(p_data->api_write.p_value) == 0x01)){
                    APPL_TRACE_ERROR0("cheng_@bta_gattc_write write is fail");
                    RTKBT_VR_not_snd_stop_request(TRUE);
                    RTKBT_VR_start_request_ok(FALSE);
                }
            }
#endif
/*BOARD_HAVE_BLUETOOTH_RTK_VR end*/
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
        ...
        if(RTKBT_VR_get_dev_cb()){
            ...
            p_rpt = bta_hh_le_find_report_entry(RTKBT_VR_get_dev_cb(),
                                        ...
                                        p_data->api_write.char_id.inst_id);
            if((p_rpt->rpt_id == rtk_voice_output_rpt_id) && (*(p_data->api_write.p_value) == 0x01)){
                APPL_TRACE_ERROR0("cheng_@bta_gattc_write write is unenque");
                RTKBT_VR_not_snd_stop_request(TRUE);
                RTKBT_VR_start_request_ok(FALSE);
            }
        }
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR begin*/
    ...
    if(RTKBT_VR_get_dev_cb()){
        tBTA_HH_LE_RPT *p_rpt;
        p_rpt = bta_hh_le_find_report_entry(RTKBT_VR_get_dev_cb(),
                                    p_clcb->p_q_cmd->api_write.srvc_id.id.inst_id,
                                    p_clcb->p_q_cmd->api_write.char_id.uuid.uu.uuid16,
                                    p_clcb->p_q_cmd->api_write.char_id.inst_id);
        APPL_TRACE_ERROR1("cheng_@bta_gattc_writecmpl p_value= 0x%02x",*(p_clcb->p_q_cmd->api_write.p_value));
        if((p_rpt->rpt_id == rtk_voice_output_rpt_id) && (*(p_clcb->p_q_cmd->api_write.p_value) == 0x01)){
            APPL_TRACE_ERROR0("cheng_@bta_gattc_write write is success");
            RTKBT_VR_start_request_ok(TRUE);
        }
    }
#endif
/*BOARD_HAVE_BLUETOOTH_RTK_VR end*/
...
#ifdef BLUETOOTH_RTK_MTU
            if((p_clcb->p_q_cmd->hdr.event == 0x5555)||(p_clcb->p_q_cmd->hdr.event == 0x5556))
            {
                if (op == GATTC_OPTYPE_CONFIG)
                    bta_gattc_RTKBT_VR_configureMTU_cmpl(p_clcb, &p_data->op_cmpl);
                else
                    APPL_TRACE_DEBUG1("cheng_@bta_gattc_op_cmpl wrong op = %d", op)
            }
            else
#endif
...
/*BOARD_HAVE_BLUETOOTH_RTK_VR_RCU_MDT Begin*/
        ...
        {
            APPL_TRACE_DEBUG0("cheng_@bta_gattc_ci_load begin to change MTU first!");
            memset(voice_mtu, 0, sizeof(tBTA_GATTC_DATA));
            voice_mtu->api_configure_mtu.configureMTU_value = RTKBT_MTU_SIZE;
            voice_mtu->api_configure_mtu.hdr.event =0x5555;
            bta_gattc_RTKBT_VR_configure_mtu(p_clcb,voice_mtu);
            GKI_freebuf(voice_mtu);
        }
        else{
            APPL_TRACE_DEBUG0("cheng_@bta_gattc_ci_load begin to get buffer fail!");
        }
        p_clcb->p_srcb->state = BTA_GATTC_SERV_IDLE;
#else
/*BOARD_HAVE_BLUETOOTH_RTK_VR_RCU_MDT end*/
        p_clcb->p_srcb->state = BTA_GATTC_SERV_DISC;
#endif  /*BOARD_HAVE_BLUETOOTH_RTK_VR_RCU_MDT*/
        p_clcb->p_srcb->attr_index = 0;
...
/*******************************************************************************
**
** Function         bta_gattc_RTKBT_VR_configure_mtu
**
** Description      configure_mtu
**
** Returns          none
**
********************************************************************************/
void bta_gattc_RTKBT_VR_configure_mtu(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_DATA *p_data)
{
    APPL_TRACE_DEBUG2("cheng_@bta_gattc_RTKBT_VR_configure_mtu conn_id=%d p_clcb->p_srcb->state = %d ",
        p_clcb->bta_conn_id, p_clcb->p_srcb->state);

    UINT16              configure_mtu = p_data->api_configure_mtu.configureMTU_value;
    tBTA_GATTC_OP_CMPL  op_cmpl;
    tBTA_GATT_STATUS    status = BTA_GATT_OK;

    if (bta_gattc_enqueue(p_clcb, p_data))
    {
        if (configure_mtu > 512 || configure_mtu < 23)
        {
            status = BTA_GATT_ERROR;
        }
        else
        {
            status = GATTC_ConfigureMTU(p_clcb->bta_conn_id, configure_mtu);
        }

        /* configureMTU fail */
        if (status != BTA_GATT_OK)
        {
            APPL_TRACE_DEBUG0("cheng_@bta_gattc_RTKBT_VR_configure_mtu fail!");
            memset(&op_cmpl, 0, sizeof(tBTA_GATTC_OP_CMPL));
...
/*******************************************************************************
**
** Function         bta_gattc_RTKBT_VR_configureMTU_cmpl
**
** Description      configure_mtu cmpl
**
** Returns          none
**
********************************************************************************/
void bta_gattc_RTKBT_VR_configureMTU_cmpl(tBTA_GATTC_CLCB *p_clcb, tBTA_GATTC_OP_CMPL *p_data)
{
    tBTA_GATTC      cb_data = {0};
    UINT8          event = BTA_GATTC_EXCHANGE_MTU_CMPL_EVT;
    APPL_TRACE_DEBUG0("cheng_@bta_gattc_RTKBT_VR_configureMTU_cmpl");
    memset(&cb_data, 0, sizeof(tBTA_GATTC));

    cb_data.cfg_mtu_cmpl.status= p_data->status;

    if (p_data->p_cmpl != NULL)
    {
        APPL_TRACE_DEBUG1("cheng_@bta_gattc_RTKBT_VR_configureMTU_cmpl mtu = %d",p_data->p_cmpl->mtu);
        cb_data.cfg_mtu_cmpl.mtu = p_data->p_cmpl->mtu;
    }
    else
    {
        APPL_TRACE_DEBUG0("cheng_@bta_gattc_RTKBT_VR_configureMTU_cmpl wrong no p_cmpl");
    }

    utl_freebuf((void **)&p_clcb->p_q_cmd);
    cb_data.cfg_mtu_cmpl.conn_id = p_clcb->bta_conn_id;
