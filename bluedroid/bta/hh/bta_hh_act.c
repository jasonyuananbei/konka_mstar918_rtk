...
void bta_hh_sdp_cmpl(tBTA_HH_DEV_CB *p_cb, tBTA_HH_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG1 ("bta_hh_sdp_cmpl:  p_cb->sec_mask: 0x%4x",p_cb->sec_mask);
#endif
    ...
    if ( status == BTA_HH_OK)
    {
        ...
        for (xx = 0; xx < BTA_HH_MAX_DEVICE; xx++)
            {
                if (!bdcmp(bta_hh_cb.kdev[xx].addr,p_cb->addr))
                {
                    break;
                }
            }
            APPL_TRACE_DEBUG3 ("bta_hh_sdp_cmpl remove bond, p_cb->vp =%d, vp =%d, p_cb->hid_handle= %d", p_cb->vp, bta_hh_cb.kdev[xx].vp,p_cb->hid_handle);
            if(bta_hh_cb.kdev[xx].vp == TRUE) {
                    bdstr_t bdstr;
                    snprintf((char*)&bdstr, sizeof(bdstr), "%02x:%02x:%02x:%02x:%02x:%02x",
                     p_cb->addr[0], p_cb->addr[1], p_cb->addr[2],p_cb->addr[3], p_cb->addr[4], p_cb->addr[5]);
                   if( !btif_config_exist("Remote",bdstr,"LinkKey")) {
                            APPL_TRACE_DEBUG0 ("bta_hh_sdp_cmpl remove bond");
                        ...
                        UINT32 data, BT_HDR *pdata)
{
    ...
    case HID_HDEV_EVT_VC_UNPLUG:
        ...
        if(xx == BTA_HH_MAX_DEVICE) {
            for (xx = 0; xx < BTA_HH_MAX_DEVICE; xx++)
            {
               if( !bdcmp(&bta_hh_cb.kdev[xx].addr,addr))
                {
                    bta_hh_cb.kdev[xx].vp = TRUE;
                    break;
                }
...
#endif
        APPL_TRACE_DEBUG3("bta_hh_cback: hidhandle = %d, bta_hh_cb.kdev[%d].vp = %d", dev_handle, xx,bta_hh_cb.kdev[xx].vp);
        break;
    }
