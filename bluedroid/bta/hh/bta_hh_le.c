    ...
    if((p_rpt->rpt_id == 1) && (p_data->len == 8) && (remote_controller_id != 0)){
        ...
        while(i < p_data->len)
        {
             if(rtkbt_getRealRcuID(remote_controller_id) == RTKBT_RCUID_IFLYTEK) {
                 if(F11_value == p_data->value[i]) {
                    APPL_TRACE_DEBUG1("pdata is 0x%x", p_data->value[i]);
                    break;
                 }
                 else if(0x45 == p_data->value[i]) {
                                 bt_bdaddr_t bd_addr;
                                 APPL_TRACE_DEBUG0("receive remove_bond key!");
                                 bdcpy(bd_addr.address, p_dev_cb->addr);
        ...
        }else if ((i < 8)&&(i  >  1)  && rtkbt_getRealRcuID(remote_controller_id) == RTKBT_RCUID_IFLYTEK)
...
/*BOARD_HAVE_BLUETOOTH_RTK_COEX end*/
	...
	if(p_rpt->rpt_id == RTKBT_IFLYTEK_INPUT_ACK || p_rpt->rpt_id == RTKBT_IFLYTEK_INPUT_DATA) {
		if ((p_buf = (UINT8 *)GKI_getbuf((UINT16)(p_data->len + 1))) == NULL)
	        ...
	        p_data->len += 1;
            if(RTKBT_send_rcu_data(RCU_VENDOR_ID, RCU_PRODUCT_ID, p_buf, p_data->len) == TRUE)
...
#ifdef BLUETOOTH_RTK
    if(!(p_rpt->rpt_id == RTKBT_IFLYTEK_INPUT_ACK || p_rpt->rpt_id == RTKBT_IFLYTEK_INPUT_DATA) ){
        if((p_dev_cb->dscp_info.vendor_id== (RCU_VENDOR_ID + 1))) {
