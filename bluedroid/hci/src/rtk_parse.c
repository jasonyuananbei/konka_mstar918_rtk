...
void update_profile_connection(RTK_CONN_PROF * phci_conn, uint16_t psm, uint8_t is_add ,uint8_t connection_type)
{
	...
	if(is_add)
	{
		...
		if(rtk_prof.profile_refcount[profile_index] == 1)
		{
			...
			//SCO/HID is always busy
			if(profile_index == profile_sco || profile_index == profile_hid)
				rtk_prof.profile_status |= BIT(profile_index);
		}
...
void udpsocket_recv(uint8_t *recv_msg, uint8_t *msg_size)
{
	...
	int n;                  /* message byte size */
	int clientlen = sizeof(rtk_prof.client_addr);

	bzero(buf, MAX_PAYLOAD);

	n = recvfrom(rtk_prof.udpsocket, buf, MAX_PAYLOAD, 0, (struct sockaddr *) &rtk_prof.client_addr, &clientlen);
	if (n < 0)
...
int create_udpsocket_socket()
{
	...
	rtk_prof.server_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	rtk_prof.server_addr.sin_port = htons((unsigned short)portno);
